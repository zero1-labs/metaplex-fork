...
var plugin$7 = require('../systemModule/plugin.cjs');
var plugin$8 = require('../tokenModule/plugin.cjs');
var plugin$9 = require('../nftModule/plugin.cjs');
var plugin$a = require('../candyMachineModule/plugin.cjs');
var plugin$b = require('../auctionHouseModule/plugin.cjs');
...
const corePlugins = () => ({
  install(metaplex) {
    ...
    metaplex.use(plugin$6.guestIdentity()); // metaplex.use(bundlrStorage());
    // Verticals.

    metaplex.use(plugin$7.systemModule());
    metaplex.use(plugin$8.tokenModule());
    metaplex.use(plugin$9.nftModule());
    metaplex.use(plugin$a.candyMachineModule());
    metaplex.use(plugin$b.auctionHouseModule());
  }
